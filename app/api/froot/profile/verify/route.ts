...
export async function GET(req: Request) {
  ...
  if (!token) {
    return NextResponse.redirect(new URL('/froot?auth=invalid', req.url))
  }

  const data = await verifyMagicToken(token)
  if (!data) {
    return NextResponse.redirect(new URL('/froot?auth=expired', req.url))
  }

  const signed = await createSession(data.profileId)
  await setSessionCookie(signed)

  return NextResponse.redirect(new URL('/froot?auth=success', req.url))
}
