...
import type { BandFit } from './sizing'

interface BandFitStepProps {
  onNext: (fit: BandFit) => void
}

const EASE: [number, number, number, number] = [0.16, 1, 0.3, 1]

const OPTIONS: { value: BandFit; label: string }[] = [
  { value: 'too_tight', label: 'Way too tight \u2014 can barely breathe' },
  { value: 'slightly_tight', label: 'A little snug \u2014 it digs in' },
  { value: 'good', label: 'Just right \u2014 snug but comfortable' },
  { value: 'too_loose', label: 'Too loose \u2014 rides up in back' },
]

export default function BandFitStep({ onNext }: BandFitStepProps) {
  const [selected, setSelected] = useState<BandFit | null>(null)
  ...
  return (
    <div style={{
      ...
      >
        the band does 80% of the work &mdash; how does yours feel right now?
      </motion.p>
